...
# Подавляем логи TensorFlow и Abseil/GLOG на уровне переменных окружения
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
os.environ['GLOG_minloglevel'] = '3'
os.environ['ABSL_LOG_SEVERITY_THRESHOLD'] = '3'

# Перенаправляем нативный C++ stderr в /dev/null
try:
    null_fds = [os.open(os.devnull, os.O_RDWR)]
    # Дублируем stderr (2) в null descriptor
        ...
        while True:
            ...
            if analysis_results.get('detected'):
                ...
                if session_data.get('exercises'):
                    ...
                    #dashboard_placeholder.pyplot(fig_dashboard, clear_figure=True)
                    # Замените строку 191 на:
                    dashboard_placeholder.pyplot(fig_dashboard, clear_figure=True, bbox_inches=None)
                    plt.close(fig_dashboard)

                # Пример: проверьте массив со статистикой (history, scores, angles и т.д.)
                history_data = st.session_state.get('history', [])

                if len(history_data) > 0:
                    # Здесь ваш старый код создания fig_dashboard
                    fig_dashboard, ax = plt.subplots()
                    ax.plot(history_data)
    
                    dashboard_placeholder.pyplot(fig_dashboard, clear_figure=True)
                    plt.close(fig_dashboard)  # Обязательно закрываем фигуру для освобождения памяти
                else:
                    # Если данных пока нет, показываем плашку или просто пропускаем
                    dashboard_placeholder.info("Ожидание данных для графика...")                
