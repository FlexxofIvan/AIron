...
class InteractiveFeedbackEvaluator(VisionLanguageEvaluator):
    ...
    def __call__(self, **sampling_kwargs):
        ...
        for it in tqdm(eval_idxs):
            ...
            # Add skeleton text if available
            if "skeleton_text" in data and data["skeleton_text"]:
                skeleton_info = f"\n\nAdditional skeletal pose information: {data['skeleton_text']}"
                input_prompt = system_prompt + skeleton_info + VISION_TOKEN
