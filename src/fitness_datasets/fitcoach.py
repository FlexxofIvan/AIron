...
def get_feedback_span(
    ...
    # Handle the case where the last feedback extends to the end
    if current_feedback is not None and start_idx is not None:
        feedback_spans.append((current_feedback, start_idx, len(feedbacks) - 1))

    # Handle mismatch between feedback spans and timestamps
    if len(feedback_spans) != len(feedback_timestamps):
        print(f"⚠️ Warning: Feedback spans ({len(feedback_spans)}) != timestamps ({len(feedback_timestamps)}).")
        
        # If we have fewer spans than timestamps, pad with empty spans
        if len(feedback_spans) < len(feedback_timestamps):
            print(f"   Padding {len(feedback_timestamps) - len(feedback_spans)} empty spans.")
            for i in range(len(feedback_timestamps) - len(feedback_spans)):
                feedback_spans.append(("", 0, 0))  # Empty feedback span
        
        # If we have more spans than timestamps, truncate
        elif len(feedback_spans) > len(feedback_timestamps):
            print(f"   Truncating {len(feedback_spans) - len(feedback_timestamps)} extra spans.")
            feedback_spans = feedback_spans[:len(feedback_timestamps)]

    return feedback_spans


def update_dataset(
    dataset: defaultdict[Any, list], updated_records: list[dict], record: dict, data_dir: str, use_skeleton_text: bool = False
) -> None:
    """
    ...
    # Path to HSMR skeleton data
    video_name = os.path.basename(record["long_range_video_file"]).replace(".mp4", "")
    skeleton_path = os.path.join(data_dir, "HSMR_outputs", f"HSMR-{video_name}.npy")
...
def load_fit_coach_dataset(
    ...
    use_skeleton_text: bool = False,
    records_filename: str = "feedbacks_long_range.json",
) -> Dataset:
    ...
    :param use_skeleton_text:
        Whether to include skeleton text descriptions as additional input.
    ...
    records_file = os.path.join(data_dir, records_filename)
    ...
    for orig_record in tqdm(workout_records):
        ...
        # Skip this record if feedback spans could not be computed
        if feedback_spans is None:
            continue
        ...
        update_dataset(dataset, updated_records, orig_record, data_dir, use_skeleton_text)
