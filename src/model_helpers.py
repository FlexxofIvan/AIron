...
def model_builder_fn(
    ...
    **kwargs,
    ...
    # Load base LLM model
    # Filter out fusion-related and bio feedback parameters that should not be passed to language model
    fusion_params = {
        'fusion_type', 'num_heads', 'fusion_dropout',
        'enable_biomech_feedback', 'biomech_feedback_weight',
        'cross_attention_weights_path',
        'lora_weights_path',
        'morphometric_dir', 'inference_speed', 'golden_standards_dir',
        'temporal_window', 'biomech_inject_mode',
    }
    llm_kwargs = {k: v for k, v in kwargs.items() if k not in fusion_params}
    llm_kwargs.update({"trust_remote_code": True})
...
def make_model(
    ...
    **model_kwargs,
    ...
    # Wrap model — pass fusion/bio params so DynamicSkeletonModelWrapper can use them
    wrapper_kwargs = {
        "device": device,
    ...
    # Pass fusion/bio params to wrapper (they were filtered from LLM kwargs in model_builder_fn)
    _fusion_keys = {
        'fusion_type', 'num_heads', 'fusion_dropout',
        'enable_biomech_feedback', 'biomech_feedback_weight',
        'cross_attention_weights_path',
    ...
    for k in _fusion_keys:
        if k in model_kwargs:
            wrapper_kwargs[k] = model_kwargs[k]
